...
class BayesianNode(DirectedNode):
    ...
    def update_down(self, message, other):
        um = ProbDist([], {})
        for m in self.umsg:
            if m.s != other:
                um = um * m.prob
    ...
    def update_up(self, message, other):
        um = ProbDist([], {})
        for m in self.umsg:
            um = um * m.prob
    ...
    def belief_propagate(self, evidence, iternum = 500, threshold = 1e-6):
        ...
        while counter < iternum:
            for m in self.messages:
                ...
                if m.mtype == 'down':
                    m.prob = m.s.update_down(m, m.e)
                elif m.mtype == 'up':
                    m.prob = m.s.update_up(m, m.e)
                
                print(counter, m.s.name, m.e.name, converged[m])
